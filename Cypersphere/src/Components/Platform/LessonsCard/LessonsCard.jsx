...
import { Trash2, FileText, ArrowRight, CheckCircle, Clock } from 'lucide-react';

function LessonsCard({ data, onDelete }) {
  ...
  const handleClick = () => {
    if (!loading) {
      navigate(`/academy/learning-path/lesson/${data.id}`);
    }
  };
  ...
  const handleDelete = async (e) => {
    ...
    setLoading(true);
    try {
      const response = await fetch(`https://cybersphere7.runasp.net/api/Lesson?id=${data.id}`, {
        method: 'DELETE',
        headers: {
  ...
  return (
    <div className="relative mb-4 group">
      ...
      >
        ...
        {/* Completion status */}
        {isCompleted && (
          <div className="absolute top-0 right-0 bg-green-500 text-white text-xs px-2 py-1 rounded-bl flex items-center">
